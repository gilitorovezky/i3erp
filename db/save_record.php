    ...
    if ( !isset($_POST["postData"]) ) { 
        ...
        if ( mysqli_connect_errno() == 0 ) { // no error from the DB
                ...
                if ( $moduleName == "projects" ) {
                    $new_project_name=$post_body[$row_number][8];
                    $sql_st = "SELECT project_name from projects where project_id = '$project_id'";
                    file_put_contents('../log/log_'.$logDate.'.log', "(save_record) ".$current_time." info 2.4-sql_st:".$sql_st."\n", FILE_APPEND);
                    ...
                    if ( $result->num_rows > 0 ) {  // project found so this is not a new project
                        ...
                        file_put_contents('../log/log_'.$logDate.'.log', "(save_record) ".$current_time." info 2.5-found project_name:".$old_ProjectName."\n", FILE_APPEND);
                    } else {
                        $project_created_date=$post_body[$row_number][9];
                        file_put_contents('../log/log_'.$logDate.'.log',"(save_record) ".$current_time." info 2.6-new project:".$new_project_name.", now create new prj folder and all sub folders\n", FILE_APPEND);
                    ...
                    switch ( $moduleName ) {
                        ...
                        case "projects"                 :
                            ...
                            if ( ( $old_ProjectName != "" ) && ( $new_project_name != $old_ProjectName ) ) { // update the project name in case the old one was changed and the oldname is not empty
                                ...
                                file_put_contents('../log/log_'.$logDate.'.log', "(save_record) ".$current_time." error 12.5-old project number is empty or no new name:".$old_ProjectName."\n", FILE_APPEND);
                                $return_code = -15;
                            }
                            $ret_recs=array("Status" => $return_code);
                        break;
